...
return new class extends Migration
{
    ...
    public function up(): void
    {
        ...
        $existingOldIds = $legacyNew->table('spendings')->pluck('old_id')->filter()->all();
        $existingOldIdsLookup = array_fill_keys($existingOldIds, true);
        ...
        $legacy->table('spendings')->orderBy('id')->chunkById(500, function (Collection $rows) use ($legacyNew, $fundMap, $itemOldMap, $contractMap, $existingOldIdsLookup): void {
            $batch = [];

            foreach ($rows as $row) {
                if (isset($existingOldIdsLookup[$row->id])) {
                ...
                // Overrides for missing items
                if ($originalItemId && isset($this->overrideItemMap[$originalItemId])) {
                    $itemId = $this->overrideItemMap[$originalItemId];
                } elseif (($originalItemId ?? null) == 34 && $row->spending_type_id && isset($this->spendingTypeMap[$row->spending_type_id])) {
                ...
                // Overrides for funds
                if (($originalItemId ?? null) == 34) {
                    $fundId = 74;
                } elseif ($row->fond_id == 9) {
                    $fundId = 8;
    ...
    public function down(): void
    {
        ...
        $existingOldIds = $legacyNew->table('spendings')->pluck('old_id')->filter()->all();
        $legacyIds = $legacy->table('spendings')->pluck('id')->all();
        $toDelete = array_values(array_intersect($legacyIds, $existingOldIds));

        if ($toDelete) {
            $legacyNew->table('spendings')->whereIn('old_id', $toDelete)->delete();
        }
    }
};
