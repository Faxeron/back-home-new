...
const tenantsEndpointUrl = tenantsEndpoint({ per_page: 200 })
const companiesEndpointUrl = companiesEndpoint({ per_page: 200 })

const { data: tenantsResponse } = await useApi<{ data: { id: number; name: string; code?: string }[] }>(tenantsEndpointUrl)
const { data: companiesResponse } = await useApi<{ data: Company[] }>(companiesEndpointUrl)
